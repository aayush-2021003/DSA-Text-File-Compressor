...
int get_code(char str[], char code[][100], int len);
struct node* decompress(struct node* root, char c);
    ...
    //Decompressing
        ...
        while((c=fgetc(file))!=EOF){
            ...
            if(n->right==NULL && n->left==NULL){
                index=match_code(code, str, len, i+1);
                printf("%d\n", index);
                //fprintf(new_file, "%c", arr[index]);
                i=0;
                n=root;
            }
            i+=1;
